...
Page({
  data: { id: '', meal: null as Meal | null, mealTypeLabel: '', status: 'loading', error: '', busy: false },
  onLoad(query: Record<string, string | undefined>) { this.setData({ id: query.id ?? '' }); void this.load() },
  async load() { this.setData({ status: 'loading', error: '' }); try { const meal = await meals.getMeal(this.data.id); this.setData({ meal, mealTypeLabel: displayLabel(meal.mealType), status: 'ready' }) } catch (error) { this.setData({ status: 'error', error: error instanceof Error ? error.message : '加载失败' }) } },
