...
<template>
  <main class="flow">
    ...
    <label>补充描述（选填）<textarea v-model="mealDraftStore.state.description" maxlength="500" placeholder="例如：鸡胸肉、米饭和西兰花，少油、酱汁另放" /></label>
    <small>AI 会结合照片，自行判断食物、份量、做法和备注。</small>
...
<style scoped>.flow{width:min(100%,32rem);margin:auto;min-height:100svh;padding:1.25rem;display:grid;align-content:start;gap:1rem}h1,p{margin:0}label{display:grid;gap:.4rem}select,button,textarea{min-height:48px;border:1px solid #dbe4e0;border-radius:.9rem;padding:.75rem .9rem;font:inherit}textarea{min-height:96px}.primary{border:0;color:#fff;background:#38c676;font-weight:700}.back{justify-self:start;border:0;background:none}.consent{display:grid;gap:.8rem;padding:1rem;border-radius:1rem;background:#eef7f3}.error{color:#a22b2b}small{color:#6a7773}</style>
