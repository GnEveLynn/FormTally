...
> 状态：已确认
> 日期：2026-09-14
> 范围：微信原生小程序客户端、微信身份登录及现有 API 的多客户端认证适配

## 1. 目标与边界

在现有 H5 和 Go API 保持可用的前提下，新增一个微信原生小程序客户端，完整覆盖以下用户闭环：

1. 用户同意当前协议后通过微信身份一键登录，无需手机号。
2. 新用户完成身体资料和每日目标设置。
...
sequenceDiagram
    ...
    MP->>API: POST /v1/auth/wechat/sessions（含协议版本）
    ...
    alt 已绑定微信身份
        ...
        API->>DB: 事务创建无手机号用户、微信身份、协议与会话
        API-->>MP: 用户信息、会话信息、token
    end
    MP->>MP: 安全保存 token
```

`loginCode` 是一次性凭证，客户端不得缓存后重复使用。AppSecret 仅存在于服务端环境变量。并发处理同一 OpenID 时由数据库事务锁串行化，避免重复或孤儿账户。

### 4.2 身份规则

服务端以 `(provider, provider_subject)` 唯一识别微信身份，其中 `provider_subject` 保存当前小程序下的 `openid`。

已有身份直接登录关联用户；新身份创建 `phone = NULL` 的独立用户。H5 手机号账户不会自动与微信账户合并。原手机号绑定接口保留供未来显式绑定使用，身份和手机号指向不同账户时继续拒绝静默归并。
...
成功响应沿用现有 session、user、consents 结构，并增加顶层 `token`；无手机号时 `phoneMasked` 为 `null`。令牌是现有不透明会话令牌，不使用 JWT。
...
CSRF 来源校验只用于 Cookie 认证的浏览器写请求。Bearer 请求不依赖 `Origin`，但必须通过会话认证。微信登录接口校验微信一次性凭证和当前协议版本；微信删号还必须用新的微信 code 复验 OpenID 归属。
...
用户拒绝协议时不创建账户。用户拒绝相机能力时仍可从相册选择。网络失败、AI 超时或上传失败时保留本次图片和表单草稿，提供重试或手动录入；会话过期时登录成功后返回原业务入口，但不在本期持久化跨进程图片草稿。
