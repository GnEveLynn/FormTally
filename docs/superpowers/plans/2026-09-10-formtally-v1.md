...
- [x] 先写月历标记、用户时区今天、月份切换与空日不创建数据的 store 测试。
- [x] 实现历史页：日历只标记正式记录；选中日期复用 `DaySummary` 展示目标、汇总和餐次。
- [x] 实现详情和完整编辑；PATCH 必须携带当前 expectedRevision。
- [x] revision 冲突时提示数据已变化并重新载入，不能静默覆盖用户值。
- [x] 使用 `affectedLocalDates` 精确刷新移日前后日期，不在客户端猜测汇总。
- [x] 删除整餐和移除图片都使用明确动词与二次确认；取消时不得发送请求。
- [x] Playwright 覆盖修改营养、移到另一日期、只删图片、取消删除、确认删餐。
...
- [x] 先写删号验证码必须属于当前用户、purpose 正确、未过期且未消费的测试。
- [x] 先写账户删除事务测试：业务数据不可再访问、所有 session 立即撤销、图片加入删除任务。
- [x] 实现 `POST /v1/account-deletions`；重复请求不泄露旧账户状态。
- [x] 实现“我的”页面及资料、目标编辑；目标结果继续复用 `GoalCalculation`。
- [x] 清楚展示修改从下一个本地日期生效，今天与历史保持原目标。
- [x] 删号页面要求验证码与二次确认；成功后清空客户端 store 并回欢迎页。
- [x] Playwright 覆盖退出、目标修改次日生效、删除账户后三条受保护页面均不可访问。
...
- [x] 为 21 个接口建立 method/path/status/请求响应 fixture 表；校验 camelCase、时间、数值精度和统一错误体。
- [x] 覆盖未认证、其他用户资源 404、非法 Origin、过期会话、过期草稿、revision 冲突、幂等冲突。
- [x] 做日志捕获测试，扫描验证码、token、Cookie、完整手机号、图片内容和完整身体资料泄漏。
- [x] 用真实 PostgreSQL 运行“首次登录 → 目标 → 分析 → 保存 → 编辑 → 历史 → 删除”集成测试；AI、短信、存储只替换外部边界。
- [x] 用 Playwright 的 Chromium、WebKit 和移动视口执行 full journey；补微信内置浏览器人工检查清单。
- [x] 检查 44×44 触控区、字号放大、字段错误关联、非颜色状态和破坏性确认。
- [x] 记录今日页 1 秒 loading 与 3 秒数据出现、AI 超过 12 秒反馈、服务端超时恢复证据。
- [x] 在 `docs/testing-v1.md` 建立 AC 编号到测试名/人工证据的可追溯矩阵，所有 72 条 AC 必须有归属。
...
- [x] 定义评测 manifest：图片引用、人工称重、参考营养、菜品类别、隐含用油标签；真实图片默认不纳入 Git。
- [x] 先写评测器测试：结构化成功率、整餐热量绝对百分比误差、中位数、p90、耗时和成本汇总。
- [ ] 对不少于 50 餐运行真实模型评测；门槛为结构成功率 ≥95%、热量 MdAPE ≤30%、p90 APE ≤60%。
- [x] 评测失败时停止发布，不通过隐藏项目或只展示总数绕过；调整模型/提示/图片引导后重新执行完整集。
- [x] 构建多阶段容器；以非 root 用户运行 Go API，加入健康检查和只读根文件系统兼容性。
- [x] 部署文档写清静态 H5、API、迁移、PostgreSQL、S3、短信、OpenAI、HTTPS、Origin 与回滚顺序。
- [ ] 在空数据库演练迁移、启动、核心 journey、回滚应用版本；数据库回滚仅在迁移明确支持时执行。
- [x] 运行生产构建并扫描产物，不得包含 `.env`、测试验证码、源图片、评测密钥或 source map 中的秘密。
- [x] 完成发布清单；只有所有自动测试、人工兼容检查、AI 门槛和外部配置均有证据时标记候选版本。
